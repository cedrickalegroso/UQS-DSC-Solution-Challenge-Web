...
app.post('/api/sendEmail', (req, res) => {
   ...
   try {
      console.log('============ using updated api [Email] ================')
      console.log('====================  version APRIL 3   =======================')
   ...
   (async () => {
      try {
         ...
         admin.firestore().collection('tickets').doc(refNo).get().then((doc) => {
            ...
            admin.firestore().doc(`tickets/${refNo}/timeline/${unixTimestamp}`)
            .set({
               message: unixTimestamp + ' ticket ' + results.ticketNo + ' transaction done.'
            })
            return true
         })
         .catch(err => console.log(err))
...
function messageNext(next) {
   ...
   admin.firestore().collection('tickets').doc(next.refNo).collection('timeline').doc(unixTimestamp).set({
      message: unixTimestamp + ' ticket ' + refNo + ' is next in-line.'
   });
...
exports.ticketCreated = functions.firestore
   ...
   .onCreate((snap, context) => {
      ...
      admin.firestore().doc(`tickets/${ticket.refNo}/timeline/${unixTimestamp}`)
      .set({
         message: unixTimestamp + ' ticket ' + ticket.ticketNo + ' has been created.'
      })
